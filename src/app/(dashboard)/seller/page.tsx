...
import { formatPrice, serializeDoc } from "@/lib/utils";
import Link from "next/link";
import { Package, ShoppingCart, TrendingUp, Eye, Plus, ArrowRight, Star, MessageCircle } from "lucide-react";
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8 flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-display text-gray-900">
            Welcome back, <span className="text-black font-bold">{session.user?.name?.split(" ")[0]}</span>
          </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-8">
        {[
          { label: "Revenue", value: formatPrice(stats.revenue), icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50 border-emerald-200" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="flex items-center justify-between p-5 border-b border-gray-100">
            ...
            <Link href="/seller/products" className="text-xs text-gray-500 hover:text-gray-900 flex items-center gap-1 transition-colors">
          ...
          {recentProducts.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <Link href="/seller/products/new" className="inline-flex items-center gap-2 px-4 py-2 text-sm text-white font-medium rounded-xl bg-black hover:bg-gray-800 transition-colors">
            ...
            <div className="divide-y divide-gray-100">
              {(recentProducts as any[]).map((p) => (
                <div key={p._id.toString()} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-10 h-10 rounded-lg bg-gray-100 overflow-hidden flex-shrink-0">
                    {p.images?.[0]?.url && <img src={p.images[0].url} alt={p.name} className="w-full h-full object-cover" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-800 truncate">{p.name}</p>
                    <p className="text-xs text-gray-400">{formatPrice(p.price)} · {p.stock} in stock</p>
                  </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="flex items-center justify-between p-5 border-b border-gray-100">
            ...
            <Link href="/seller/orders" className="text-xs text-gray-500 hover:text-gray-900 flex items-center gap-1 transition-colors">
              View all <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          {orders.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-gray-400">No orders yet. Publish products to start selling!</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-100">
              {(orders as any[]).map((o) => {
                ...
                return (
                  <div key={o._id.toString()} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800 truncate">{o.userName}</p>
                      <p className="text-xs text-gray-400">{myItems.length} item{myItems.length !== 1 ? "s" : ""} · {new Date(o.createdAt).toLocaleDateString()}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-semibold text-gray-800">{formatPrice(myTotal)}</p>
                      <span className={`text-xs font-medium capitalize ${
                        o.status === "paid" || o.status === "delivered" ? "text-emerald-600" :
                        o.status === "shipped" ? "text-blue-600" :
                        o.status === "cancelled" ? "text-red-500" : "text-amber-600"
                      }`}>
                        {o.status}
                      </span>
                    </div>
